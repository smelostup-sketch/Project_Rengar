...
func _update_player(_delta: float) -> void:
	# 🔑 Синхронизация: когда animation завершилась, завершаем боевой цикл
	if is_recovery:
		recovery_timer -= _delta
		if recovery_timer <= 0.0:
			is_recovery = false
			attack_finished.emit()  # ✅ ДОБАВЛЕНО: сигнал о завершении атаки
			_trace("ATTACK_SEQUENCE_COMPLETE | animation_finished")
