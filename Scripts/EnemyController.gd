...
# Намерение движения (ранее жило в CombatComponent.ai_state).
var move_state := "IDLE"
...
func _physics_process(delta: float) -> void:
	...
	if combat_comp.is_recovery:
		# Атакующий стоит: новое решение не накладывается на незавершённое действие.
		move_state = "STOPPED"
	elif combat_comp.ai_state != "STUNNED":
		if dist > detection_range or player_is_stealthed:
			...
			# Базовая логика без ИИ: преследование и случайные атаки.
			move_state = "CHASE" if dist > stop_distance else "STOPPED"
	
	# Поворот к игроку
	if not combat_comp.is_recovery and is_instance_valid(player):
	...
	if move_state == "CHASE":
		...
		if utility_is_active and utility_ai.wants_block():
		...
		elif dist <= 3.0 and not combat_comp.is_recovery and not combat_comp.is_input_locked and not combat_comp.is_block_active:
			# Намерение выражается только когда ядро свободно: гард ядра остаётся
			# последней инстанцией, но провайдер не спамит отказами каждый кадр.
			...
			if not player_leaping and (not utility_is_active or utility_ai.wants_attack()):
				# Единое ядро: намерение -> perform; ядро само отказывает во время
				# recovery/lock/блока, поэтому перезапусков-подмен не бывает.
				var dirs := ["UP", "LEFT", "RIGHT"]
				var attack_direction := utility_ai.get_recommended_attack_direction() if utility_is_active else ""
				if attack_direction == "":
					attack_direction = dirs[randi_range(0, 2)]
				combat_comp.perform_directional_attack(attack_direction, false)
	else:
		velocity.x = 0.0
		velocity.z = 0.0
...
func take_dmg(amount: float, attacker_pos: Vector3 = Vector3.ZERO) -> void:
	if health_comp and health_comp.is_dead:
		return
	if combat_comp and combat_comp.is_input_locked:
		return
	_play_hit_flash()
...
func _show_telegraph() -> void:
	if not telegraph_label or combat_comp.current_attack_dir == "":
		return
	if combat_comp.current_attack_dir == "UP":
		telegraph_label.text = "↑"
	elif combat_comp.current_attack_dir == "LEFT":
		telegraph_label.text = "←"
	elif combat_comp.current_attack_dir == "RIGHT":
		telegraph_label.text = "→"
...
func _update_telegraph_visibility() -> void:
	if combat_comp.is_recovery and not combat_comp.attack_swing_active:
		_show_telegraph()
	else:
		_hide_telegraph()
...
func _update_enemy_indicator() -> void:
	...
	if combat_comp.enemy_block_active:
		...
		lbl.text = "💫"
	elif combat_comp.is_recovery and not combat_comp.attack_swing_active:
		lbl.text = "⚡"
	else:
		if move_state == "IDLE":
			lbl.text = "❓"
		elif move_state == "CHASE":
			lbl.text = "❗"
		elif move_state == "STOPPED":
			lbl.text = "🛑"
	var cam = get_viewport().get_camera_3d()
