...
func wants_block() -> bool:
	return current_decision == Decision.HOLD and perception != null and perception.get_player_attack_recency(0.8) > 0.25
...
func _calculate_scores() -> Dictionary:
	...
	return {
		...
		Decision.HOLD: near_preferred * 0.28 + player_blocking * 0.32 + block_pressure * 0.90,
