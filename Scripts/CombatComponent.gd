...
## Единое боевое ядро владельца (CharacterBody3D): игрок и враг воюют ОДНОЙ логикой.
## Контракт для провайдеров намерений (AttackInputComponent игрока,
## EnemyController/AI врага, будущий ИИ):
##   НАМЕРЕНИЕ: perform_directional_attack(dir, leap) -> bool
##     (ядро отказывает во время recovery / input lock / блока — решение
##      провайдера не может накладываться на незавершённое действие)
##   ИСХОДЫ (сигналы, на которые подписывается провайдер и анимация):
##     attack_started / attack_performed / attack_finished / attack_cancelled /
##     hit_detected / block_successful / stun_started / damage_taken
## Урон, хитбоксы и отмены владеют состоянием только здесь; визуал — в
## анимационных компонентах через сигналы; окна хитбоксов — Method Track'и клипов.
...
# === WEAPON HITBOX DEBUG ===
@export_group("Weapon Hitbox Debug")
@export var debug_hitboxes_enabled: bool = false
@export var debug_hitbox_key: Key = KEY_F8
@export var debug_hitbox_color: Color = Color(1.0, 0.0, 0.0, 0.35)
...
# Ожидаемый поток управления (отказы занятого ядра) — только в verbose-режиме.
...
# === СОСТОЯНИЕ ВРАГА ===
# ai_state больше не владеет атакой: только STUNNED/IDLE (стан).
# Намерения движения (CHASE/STOPPED/RETREAT) живут в EnemyController.
var ai_state: String = "IDLE"
var stun_timer: float = 0.0
# true, пока Method Track'и клипа держат окно хитбокса открытым
# (телеграф врага читает этот флаг: телеграф = recovery && !attack_swing_active).
var attack_swing_active: bool = false
...
func _ready() -> void:
	...
	print("========== WEAPON HITBOX DEBUG ==========")

	for i in range(weapon_hitboxes.size()):
		var hitbox: Area3D = weapon_hitboxes[i]
		print(
			"weapon_hitboxes[",
			i,
			"] = ",
	...
	if weapon_hitboxes.is_empty():
		# Манекен-мишень не атакует по замыслу: отсутсвие хитбоксов для него норма.
		if owner_body.is_in_group("dummy_enemy"):
			_trace("WEAPON_HITBOX_SKIP | owner=dummy (не атакует)")
		else:
			push_error("CombatComponent: не найдено ни одного WeaponHitbox!")

	# DEBUG: если включено в Inspector, сразу создаём визуализацию.
	if debug_hitboxes_enabled:
		set_debug_hitboxes_visible(true)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode == debug_hitbox_key:
			set_debug_hitboxes_visible(not debug_hitboxes_enabled)

func _update_timers(delta: float) -> void:
	# Recovery — единый каденс атаки игрока и врага; ядро — источник правды.
	if is_recovery:
		recovery_timer -= delta
		if recovery_timer <= 0.0:
			is_recovery = false
			attack_finished.emit()
			_trace("ATTACK_SEQUENCE_COMPLETE | recovery_end")
	if is_input_locked:
		lock_timer -= delta
		if lock_timer <= 0.0:
...
# === ЕДИНАЯ ПОДТВЕРЖДЁННАЯ НАПРАВЛЕННАЯ АТАКА ===
# Игрок: вызывается AttackInputComponent после выбора направления и задержки.
# Враг: вызывается EnemyController (провидер намерений; ИИ или базовая логика).
# Ядро владеет уроном и хитбоксом; провайдер владеет только вводом/намерением.
func perform_directional_attack(direction: String, is_leap_attack: bool = false) -> bool:
	if is_input_locked or is_block_active or is_recovery:
		_vtrace("ATTACK_REJECTED | reason=locked_or_recovery")
		return false
...
func cancel_attack(reason: String) -> void:
...
# Area3D создан один раз в сцене как дочерний узел BoneAttachment3D.
...
func activate_enemy_block(block_direction: String = "UP") -> bool:
	if is_player or not enemy_block_enabled or owner_body == null:
		return false
	if is_recovery or ai_state == "STUNNED":
		return false
...
func player_take_dmg(amount: float, attacker_pos: Vector3 = Vector3.ZERO) -> void:
	...
	cancel_attack("damage_interrupt")
	if health_comp:
...
# === ВРАГ: УРОН И БЛОК ===
# Атаку враг начинает через единый perform_directional_attack() из EnemyController.
# Параллельного автомата WINDUP/SWING больше нет: жизненный цикл атаки один на всех.
func enemy_on_block() -> void:
	_trace("ENEMY_ATTACK_CONTACT | kind=block | dir=" + current_attack_dir)
	cancel_attack("blocked")
	apply_stun(0.5)

func enemy_on_take_dmg(_amount: float, attacker_pos: Vector3 = Vector3.ZERO) -> void:
	# Единое правило с игроком (player_take_dmg): урон отменя текущую атаку
	# одним путём (состояние + хитбокс + визуал), затем короткий input lock,
	# чтобы следующее НАМЕРЕНИЕ атаки не прилетело мгновенно.
	cancel_attack("damage_interrupt")
	is_input_locked = true
	lock_timer = attack_delay_duration
	damage_taken.emit()
...
func _vtrace(message: String) -> void:
	if debug_logging and verbose_logging:
		print("[Combat] ", message)

# 🔧 DEBUG ФУНКЦИЯ (только в тестировании)
func get_debug_timers() -> Dictionary:
	return {
		"recovery_active": is_recovery,
...
# ============================================================
# WEAPON HITBOX DEBUG VISUALIZATION
# ============================================================

func set_debug_hitboxes_visible(enabled: bool) -> void:
	debug_hitboxes_enabled = enabled

	for shape in weapon_hitbox_shapes:
		if not is_instance_valid(shape):
			continue

		var debug_mesh := shape.get_node_or_null("DebugHitbox") as MeshInstance3D

		if enabled:
			if debug_mesh == null:
				debug_mesh = _create_debug_hitbox(shape)

			debug_mesh.visible = true

		elif debug_mesh != null:
			debug_mesh.visible = false


func _create_debug_hitbox(shape: CollisionShape3D) -> MeshInstance3D:
	var mesh_instance := MeshInstance3D.new()
	mesh_instance.name = "DebugHitbox"
	mesh_instance.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF

	var material := StandardMaterial3D.new()
	material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	material.albedo_color = debug_hitbox_color
	material.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED

	var shape_resource := shape.shape

	if shape_resource == null:
		push_warning(
			"DebugHitbox: CollisionShape3D не имеет Shape: "
			+ str(shape.get_path())
		)
	...
	if shape_resource is BoxShape3D:
		...
		box_mesh.size = shape_resource.size
		mesh_instance.mesh = box_mesh

	elif shape_resource is CapsuleShape3D:
		var capsule_mesh := CapsuleMesh.new()
		capsule_mesh.radius = shape_resource.radius
		capsule_mesh.height = shape_resource.height
		mesh_instance.mesh = capsule_mesh

	elif shape_resource is SphereShape3D:
		var sphere_mesh := SphereMesh.new()
		sphere_mesh.radius = shape_resource.radius
		sphere_mesh.height = shape_resource.radius * 2.0
		mesh_instance.mesh = sphere_mesh

	else:
		push_warning(
			"DebugHitbox: неподдерживаемый Shape: "
			+ shape_resource.get_class()
		)
		mesh_instance.queue_free()
		return mesh_instance
