...
func _render_debug_lines() -> void:
	...
	var has_player_hitbox := is_instance_valid(player_candidate)
	var has_enemy_hitbox := is_instance_valid(enemy_candidate)
...
func _draw_mouse_direction_ray() -> void:
	if player_body == null or attack_input == null:
		return
