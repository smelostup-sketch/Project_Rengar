...
var debug_line: Label
@export var debug_line_enabled := true
...
func _update_debug_line() -> void:
	...
	var input_comp: AttackInputComponent = player.get_node_or_null("AttackInputComponent") as AttackInputComponent if player != null else null
	if input_comp == null:
		return
	var viewport_size := get_viewport().get_visible_rect().size
	debug_line.position = Vector2(maxf(8.0, viewport_size.x - 640.0), viewport_size.y - 34.0)
	debug_line.size = Vector2(minf(632.0, viewport_size.x - 16.0), 24.0)
	var direction := input_comp.get_debug_direction()
	if direction.is_empty():
		direction = "—"
	var delta := input_comp.get_debug_mouse_delta()
	debug_line.text = "DEBUG INPUT: %s | dir: %s | mouse: (%.1f, %.1f)" % [input_comp.get_state_name(), direction, delta.x, delta.y]
...
func _build_ui():
	...
	debug_line = Label.new()
	debug_line.name = "DebugInputLine"
	debug_line.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	debug_line.add_theme_font_size_override("font_size", 14)
	debug_line.add_theme_color_override("font_color", Color(1.0, 0.92, 0.35, 1.0))
	debug_line.add_theme_color_override("font_shadow_color", Color(0.0, 0.0, 0.0, 0.9))
	debug_line.add_theme_constant_override("shadow_offset_x", 2)
	debug_line.add_theme_constant_override("shadow_offset_y", 2)
