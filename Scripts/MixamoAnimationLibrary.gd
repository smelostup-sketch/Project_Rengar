...
# Нормализованные точки внутри каждого FBX-клипа. Эти Method Track являются
# единственным gameplay-источником включения/выключения статического Area3D меча.
#
# КАЛИБРОВКА (q_tools/measure_clips.gd, угловая скорость кости атакующей руки):
# Правило: окно открывается незадолго до ядра свинга (порог 0.7 от пика) и
# закрывается после конца полосы свинга (порог 0.5) с небольшим хвостом, чтобы
# первое пересечение хитбокса с целью приходилось на фактическую фазу удара,
# а не на замах (регистрация удара однократная на взмах).
#   AT_LEFT  (2.667s): LeftHand  ядро 0.45..0.55, полоса 0.41..0.55 -> 0.40..0.58
#   AT_RIGHT (3.833s): RightHand ядро 0.11..0.26, полоса 0.08..0.48 -> 0.10..0.50
#   AttackUp играет клип AT_RIGHT, поэтому окно совпадает с AttackRight.
const WEAPON_HITBOX_WINDOWS := {
	&"AttackLeft": Vector2(0.40, 0.58),
	&"AttackRight": Vector2(0.10, 0.50),
	&"AttackUp": Vector2(0.10, 0.50),
}
...
func _add_clip(target_player: AnimationPlayer, library: AnimationLibrary, animation_name: StringName, source_path: String) -> void:
	...
	if animation_name == &"Idle" or animation_name == &"Run":
		...
		_add_weapon_hitbox_method_track(copy, animation_name)
...
func _add_weapon_hitbox_method_track(clip: Animation, animation_name: StringName) -> void:
	if not WEAPON_HITBOX_WINDOWS.has(animation_name):
		return
	var window: Vector2 = WEAPON_HITBOX_WINDOWS[animation_name]
	var enable_time := clampf(clip.length * window.x, 0.0, maxf(clip.length - 0.02, 0.0))
	var disable_time := clampf(clip.length * window.y, enable_time + 0.01, clip.length)
	var track_index := clip.add_track(Animation.TYPE_METHOD)
	# root_node AnimationPlayer равен ".." (VisualModel/EnemyVisual), поэтому
	# CombatComponent как sibling модели разрешается одним переходом вверх.
	clip.track_set_path(track_index, NodePath("../CombatComponent"))
	clip.track_insert_key(track_index, enable_time, {"method": &"animation_enable_weapon_hitbox", "args": []})
	clip.track_insert_key(track_index, disable_time, {"method": &"animation_disable_weapon_hitbox", "args": []})
